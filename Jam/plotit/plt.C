...
void plt(){
  ...
  ifstream readme; 
  std::string cmddraw="xxxxxxxxxxxxxxx";
  ...
  for(int iii=0; iii<10000; iii++){  
    ...
    if(cmds1=="d" ||cmds1=="df" || cmds1=="dd" ||cmds1=="ddf" ){
      // draw 
    ...
    if(cmds1=="def"){
      // print defaults
      cout <<" defaults "<<endl;
      cout<<" draw command cmddraw for n and p = "<<cmddraw<<endl;
    ...
    if(drawit){
      cout<<" defpad ="<< defpad<<endl;
      if(cmds1 =="n" || cmds1 =="p"){
	cmds1=cmddraw;
	//	cout<<" the draw command is: "<<cmds1<<endl;
      }
      if(cmds1 =="d" || cmds1 =="df"|| cmds1 =="dd"|| cmds1 =="ddf"){
	cmddraw=cmds1;
	cout<<" draw command now set to: "<<cmddraw<<endl;
      }
      ...
      string titlestringxy="xvsy_pr_density_"+histstr+";1";
      ...
      TProfile2D *histdens;
      histdens = (TProfile2D*) gFile->Get(titlechrxy);
      if (gDirectory->FindObject(histdens)) printf ("%s exists \n", titlechrxy); else {printf ("%s doesn't exist \n", titlechrxy); return;}

      histprof->GetXaxis()->SetRangeUser(xlow,xhigh);
      histprof->GetYaxis()->SetRangeUser(ylow,yhigh);

      histdens->GetXaxis()->SetRangeUser(xlow,xhigh);
      histdens->GetYaxis()->SetRangeUser(ylow,yhigh);

      canv[canvnumb]->cd(defpad);

      if(cmds1=="d" || cmds1=="df")histprof->DrawCopy("ARR,COLZ");      

      if(cmds1=="dd")histdens->DrawCopy("CONT4");
      
      if(cmds1=="ddf"){
	histdens->DrawCopy("CONT4");
	histprof->DrawCopy("ARR,SAME");
      }
      
      // cant get marker to work. Just draws it in center of plot, not at 0,0   
      /*
      TMarker *centr = new TMarker(0,0, 8);
      centr->SetMarkerColorAlpha(kRed, 1.0);
      centr->SetMarkerSize(2);
      centr->Draw();   
      */
