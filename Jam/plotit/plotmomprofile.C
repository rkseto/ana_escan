...
void plotmomprofile (string cmdstr = "HELP",string filestr = "jam1_3_b0-14_RQMDS_all.root", string histstr = "xvsyvspt_mid_bin4", double ptmin=0., double ptmax=2. , double axissize=30.) {
  ...
  if(cmdstr == "DUMP"){
    ...
    ofstream momoutfile;
    momoutfile.open("momoutfile.txt");
    for(int i=1; i<nx+1; i++){
      for(int j=1; j<ny+1; j++){
  ...
  if(cmdstr == "DUMPSLICE"){
   ...
   string hist3Dpxstr="rvsphivspr_3D_"+histstr;
    ...
    TH2D *prof2Dpt = new TH2D("profile_xvsy_pt","profile_xvsy_pt",nx,xmin,xmax,ny,ymin,ymax);
    ofstream mom3Doutfile;
    mom3Doutfile.open("mom3Doutfile.txt");
	...
	prof2Dpt->SetBinContent(i,j,sqrt(contentpx*contentpx+contentpy*contentpy));
	...
	mom3Doutfile<<x<<" "<<y<<" "<<px<<" "<<py<<endl;	
      }
    }
    mom3Doutfile.close();
    TFile *myfile = new TFile("plotmomprofile.root","RECREATE");
    myfile->cd();
    prof2Dpx->Write();
    prof2Dpy->Write();
    prof2Dpt->Write();
