...
void plotallmomprofiles (string filestr = "jam1_3_b0-14_RQMDS_all.root", double ptmin=0., double ptmax=5. ) {
  ...
  TProfile3D *hist3Dxy;  // for reasons I don't understand, this works for any histogram
  //  TH3D *hist3Dxy;  // for reasons I don't understand, this works for any histogram
  ...
  for (int icent=0; icent<3; icent++){
    for (int irapbin=0; irapbin<9; irapbin++){
      ...
      string titlestringxy="xvsy_pr_density_"+histstr;
      ...
      strcpy(titlechrxy,titlestringxy.c_str());
      
      fin->cd();  
      cout<<" DUMP a SLICE in pt"<<endl;
      cout << "Input hist:  " << histstr << endl;
      
      string hist3Dxystr="xvsyvspt_"+histstr;
      cout<<" **** hist3Dxystr="<<hist3Dxystr<<endl;
      char hist3Dxychr[50];
      strcpy(hist3Dxychr,hist3Dxystr.c_str());
      hist3Dxy = (TProfile3D*) fin->Get(hist3Dxychr);
      if (gDirectory->FindObject(hist3Dxy)) printf ("%s exists \n", hist3Dxychr); else {printf ("%s doesn't exist \n", hist3Dxychr); return;} 
      ...
      int xynx=hist3Dxy->GetNbinsX();
      int xyny=hist3Dxy->GetNbinsY();
      int xynz=hist3Dxy->GetNbinsZ();
      double xydx = hist3Dxy->GetXaxis()->GetBinCenter(2)-hist3Dxy->GetXaxis()->GetBinCenter(1);
      double xyxmin=hist3Dxy->GetXaxis()->GetBinCenter(1)-xydx/2.;
      double xyxmax=hist3Dxy->GetXaxis()->GetBinCenter(xynx)+xydx/2.;
      cout<<" **** for checking xynx xyxmin xyxmax="<<xynx<<" "<<xyxmin<<" "<<xyxmax<<endl;
      cout<<" **** xynz="<<xynz<<endl;      
      ...
      int xrebin=1;
      int yrebin=1;
      if(nx != xynx)xrebin=xynx/nx;
      if(ny != xyny)yrebin=xyny/ny;
      if(xrebin != 1 || yrebin != 1) {cout<<" rebinning 3D xy hist factors= "<<xrebin<<" "<<yrebin<<endl; hist3Dxy->Rebin3D(xrebin, yrebin, 1);}     
      ...
      prof2Dxy_ar[icentrap] = new TH2D(titlechrxy,titlechrxy,nx,xmin,xmax,ny,ymin,ymax);

      //       prof2Dxy_ar[icentrap] = hist3Dxy->Project3DProfile("xy");
	...
	for(int k=iptmin; k<iptmax+1; k++){
	  content+=hist3Dxy->GetBinContent(i,j,k);
	}
        prof2Dxy_ar[icentrap]->SetBinContent(i,j,content);
      }
    }
       
       
      prof2Dxy_ar[icentrap]->SetNameTitle(titlechrxy,titlechrxy);
